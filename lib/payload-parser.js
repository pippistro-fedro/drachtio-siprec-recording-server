...
module.exports = function parseSiprecPayload(opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    xmlParser(meta, (err, result) => {
      ...
      const mLinesPerLeg = (sdp.match(/^m=/gm) || []).length / 2;
      const arr = /^([^]+?)(m=[^]+)$/.exec(sdp);
      const sdpFirstPart = arr[1];
      const mLines = arr[2];
      ...
      mLines.split('\r\n').forEach((line) => {
        if (line.startsWith('m=')) {
          sdpx = (i < mLinesPerLeg) ? 'sdp1' : 'sdp2';
          if (!opts[sdpx]) {
            opts[sdpx] = sdpFirstPart;
          }
          i++;
        }
        opts[sdpx] += `${line}\r\n`;
  ...
  function parseCallData(prefix, obj) {
    ...
    if (group) {
      ...
      const s = obj['session'];
      if (s && s.length && s[0].sipSessionID && s[0].sipSessionID.length) {
        ret['callid'] = s[0].sipSessionID[0];
      }
    }
